...
#' @export
#'
rpt_shiny <- function(appname = "rank", pkgname = "rankpts") {

    req_pkgs        <- c("shiny", "DT", "bsicons", "bslib")
    chk_uninstalled <- vapply(req_pkgs,
    ...
    if (app_dir == "") {
        stop("Could not find the Shiny app directory. Try re-installing the `",
             pkgname, "` package.", call. = TRUE)
    }
